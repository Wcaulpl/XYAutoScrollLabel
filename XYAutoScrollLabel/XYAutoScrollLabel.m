...
@implementation XYAutoScrollLabel
...
- (void)commonInit {
    ...
    self.maxWidth = CGRectGetWidth(self.frame);
    if (!self.maxWidth) {
        self.maxWidth = CGRectGetWidth([UIScreen mainScreen].bounds);
    }
    self.minHeight = CGRectGetHeight(self.frame);
...
- (void)setText:(NSString *)theText refreshLabels:(BOOL)refresh {
    ...
    CGRect newRect = self.mainLabel.bounds;
    if (CGRectGetWidth(self.mainLabel.bounds) > self.maxWidth) {
        newRect = CGRectMake(0, 0, self.maxWidth, CGRectGetHeight(newRect));
    }
    if (CGRectGetHeight(self.mainLabel.bounds) < self.minHeight) {
        newRect = CGRectMake(0, 0, CGRectGetWidth(newRect), self.minHeight);
    }
